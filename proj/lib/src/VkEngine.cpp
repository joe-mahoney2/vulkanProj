...
VkEngine::VkEngine() {
	...
	if (initSwapChain()) { std::cout << "VkEngine::initSwapChain SUCCESS!" << std::endl; }
    if (initRenderData()) { std::cout << "VkEngine::initRenderData SUCCESS!" << std::endl; }
	if (initGfxPipeline()) { std::cout << "VkEngine::initGfxPipeline SUCCESS!" << std::endl; }
	if (initFrameBuffer()) { std::cout << "VkEngine::initFrameBuffer SUCCESS!" << std::endl; }
	if (initCmdPool()) { std::cout << "VkEngine::initCmdPool SUCCESS!" << std::endl; }
	if (initCmdBuffer()) { std::cout << "VkEngine::initCmdBuffer SUCCESS!" << std::endl; }
	if (initSyncObjects()) { std::cout << "VkEngine::initSyncObjects SUCCESS!" << std::endl;}
...
int VkEngine::initGfxPipeline() {
	...
	if (vkCreatePipelineLayout (
	        vkCtl.vkbDevice, &pipeline_layout_info, nullptr, &vkRen.pipeLayout) != VK_SUCCESS) {
	...
	if (vkCreateGraphicsPipelines (
	        vkCtl.vkbDevice, VK_NULL_HANDLE, 1, &pipeline_info, nullptr, &vkRen.gfxPipe) != VK_SUCCESS) {
...
int VkEngine::initFrameBuffer() {
	...
	for (size_t i = 0; i < vkRen.scImageViews.size (); i++) {
		...
		if (vkCreateFramebuffer (vkCtl.vkbDevice, &framebuffer_info, nullptr, &vkRen.frameBuffers[i]) != VK_SUCCESS) {
			return FAIL;
		}
	}
	return SUCCESS;
...
int VkEngine::initCmdPool() {
	...
	pool_info.queueFamilyIndex = vkCtl.vkbDevice.get_queue_index (vkb::QueueType::graphics).value ();

	if (vkCreateCommandPool (vkCtl.vkbDevice, &pool_info, nullptr, &vkRen.cmdPool) != VK_SUCCESS) {
...
int VkEngine::initCmdBuffer() {
	...
	if (vkAllocateCommandBuffers (vkCtl.vkbDevice, &allocInfo, vkRen.cmdBuffers.data ()) != VK_SUCCESS) {
	...
	for (size_t i = 0; i < vkRen.cmdBuffers.size (); i++) {
		...
		if (vkBeginCommandBuffer (vkRen.cmdBuffers[i], &begin_info) != VK_SUCCESS) {
...
int VkEngine::initSyncObjects() {
	...
	for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
		if (vkCreateSemaphore (vkCtl.vkbDevice, &semaphore_info, nullptr, &vkRen.available_semaphores[i]) != VK_SUCCESS ||
		    vkCreateSemaphore (vkCtl.vkbDevice, &semaphore_info, nullptr, &vkRen.finished_semaphore[i]) != VK_SUCCESS ||
		    vkCreateFence (vkCtl.vkbDevice, &fence_info, nullptr, &vkRen.in_flight_fences[i]) != VK_SUCCESS) {
...
int VkEngine::recreateSwapChain() {
	...
	vkDestroyCommandPool (vkCtl.vkbDevice, vkRen.cmdPool, nullptr);

	for (auto framebuffer : vkRen.frameBuffers) {
		vkDestroyFramebuffer (vkCtl.vkbDevice, framebuffer, nullptr);
	}
...
int VkEngine::drawFrame() {
	vkWaitForFences (vkCtl.vkbDevice, 1, &vkRen.in_flight_fences[vkRen.current_frame], VK_TRUE, UINT64_MAX);

	uint32_t image_index = 0;
	VkResult result = vkAcquireNextImageKHR (vkCtl.vkbDevice,
	    vkCtl.swapchain,
	    UINT64_MAX,
	...
	if (vkRen.image_in_flight[image_index] != VK_NULL_HANDLE) {
		vkWaitForFences (vkCtl.vkbDevice, 1, &vkRen.image_in_flight[image_index], VK_TRUE, UINT64_MAX);
	}
	vkRen.image_in_flight[image_index] = vkRen.in_flight_fences[vkRen.current_frame];

	VkSubmitInfo submitInfo = {};
	submitInfo.sType = VK_STRUCTURE_TYPE_SUBMIT_INFO;

	VkSemaphore wait_semaphores[] = { vkRen.available_semaphores[vkRen.current_frame] };
	...
	VkSemaphore signal_semaphores[] = { vkRen.finished_semaphore[vkRen.current_frame] };
	submitInfo.signalSemaphoreCount = 1;
	submitInfo.pSignalSemaphores = signal_semaphores;

	vkResetFences (vkCtl.vkbDevice, 1, &vkRen.in_flight_fences[vkRen.current_frame]);

	if (vkQueueSubmit (vkRen.gfxQueue, 1, &submitInfo, vkRen.in_flight_fences[vkRen.current_frame]) != VK_SUCCESS) {
...
VkShaderModule VkEngine::createShaderModule(const std::vector<char>& code) {
	...
	if (vkCreateShaderModule(vkCtl.vkbDevice, &create_info, nullptr, &shaderModule) != VK_SUCCESS) {
